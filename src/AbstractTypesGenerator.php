 ...
 */
abstract class AbstractTypesGenerator implements TypesGeneratorInterface
{
    /**
     * @var string
     *
    ...
    public function generate(array $config): void
    {
        ...
        foreach ($typesToGenerate as $typeName => $type) {
            ...
            if ($class['isEnum']) {
                $this->configureEnum($class, $type, $typeConfig, $config);
            } else {
                $this->configureClass($class, $type, $typeName, $typeConfig, $config);
            }
    ...
    public function generateAnnotations(&$classes, array $typesToGenerate, array $config): void
    {
    ...
    public function render(array $classes, array $config)
    {
        $interfaceMappings = [];
        $generatedFiles = [];
     ...
     */
    public function getTypesToGenerate(array $config)
    {
        $typesToGenerate = [];

        if (!$config['types']) {
            foreach ($this->graphs as $graph) {
                $typesToGenerate = $graph->allOfType($this->resourceTypes['class']);
            }
        ...
        foreach ($config['types'] as $typeName => $typeConfig) {
            ...
            foreach ($this->graphs as $graph) {
                ...
                if (isset($resources[$typeConfig['vocabularyNamespace'].$typeName])) {
                    $resource = $graph->resource($typeConfig['vocabularyNamespace'].$typeName, $this->resourceTypes['class']);
                    break;
                }
            }
            ...
            if ($resource) {
                ...
                if (isset($graph)) {
                    $type = $graph->resource($typeConfig['vocabularyNamespace'].$typeConfig['guessFrom'], $this->resourceTypes['class']);
    ...
    public function configureEnum(&$class, $type, array $typeConfig, array $config)
    {
    ...
    public function configureClass(&$class, $type, $typeName, $typeConfig, $config)
    {
        // Entities
        ...
        if (null === $class['parent']) {
            $numberOfSupertypes = count($type->all($this->resourceTypes['subClass']));

            if ($numberOfSupertypes > 1) {
                $this->logger->warning(sprintf('The type "%s" has several supertypes. Using the first one.', $type->localName()));
            }

            $class['parent'] = $numberOfSupertypes ? $type->all($this->resourceTypes['subClass'])[0]->localName() : false;
        }
    ...
    public function configureProperties(&$class, $type, $typeName, $typeConfig, $propertiesMap, $config)
    {
        // Fields
    ...
    public function configureIdGenerator(&$classes, array $config)
    {
        foreach ($classes as &$class) {
    ...
    {
        $subClassOf = $type->get($this->resourceTypes['subClass']);
     ...
     * @return array
     */
    protected function getParentClasses(\EasyRdf_Resource $resource, array $parentClasses = [])
    {
        if ([] === $parentClasses) {
            return $this->getParentClasses($resource, [$resource->getUri()]);
        }

        $subclasses = $resource->all($this->resourceTypes['subClass']);
        ...
        foreach ($this->graphs as $graph) {
            foreach ($graph->allOfType($this->resourceTypes['class']) as $type) {
    ...
    {
        return $property->all(self::SCHEMA_ORG_RANGE);
    }
    ...
    private function generateField(array $config, array $class, \EasyRdf_Resource $type, $typeName, $propertyName, \EasyRdf_Resource $property = null)
    {
        ...
        if (!empty($property) && preg_match('/legacy spelling/', (string) $property->get($this->resourceTypes['comment']))) {
        ...
        if (isset($propertyConfig['range']) && $propertyConfig['range']) {
            ...
            foreach ($this->getPropertyRange($property) as $range) {
        ...
        if (empty($ranges) && !empty($this->defaultRange)) {
            $ranges[] = $this->defaultRange;
        }
